...
func test_guard_will_not_block_with_a_frozen_banner():
	var g := Scenario.fresh()
	var pol := AIPolicy.new("guard")
	Scenario.set_life(g, 1, 1)  # low life -> guard wants to block
...
func test_guard_blocks_with_a_ready_blocker():
	var g := Scenario.fresh()
	var pol := AIPolicy.new("guard")
...
func test_guard_targets_a_rested_enemy_banner_it_can_beat():
	var g := Scenario.fresh()
	var pol := AIPolicy.new("guard")
	...
	assert_eq(target, rested, "guard clears the beatable rested Banner")
